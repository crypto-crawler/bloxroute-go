...
// doc: https://docs.bloxroute.com/streams/bdnblocks#examples

// Subscribe to the `bdnBlocks` stream from bloXroute gateway or cloud API.
func main() {
	...
	gatewayUrl := flag.String("gateway", os.Getenv("BLOXROUTE_GATEWAY_URL"), "The gateway url")
	header := flag.String("header", os.Getenv("AUTHORIZATION_HEADER"), "The authorization header")
	flag.Parse()
	if *gatewayUrl == "" {
		if *certFile == "" || *keyFile == "" {
